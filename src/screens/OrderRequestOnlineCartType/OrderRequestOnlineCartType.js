...
import { orderRequestFinishOrderAction, finishOrderAction } from '../../redux/Cart/Actions';
import { useDispatch, useSelector } from 'react-redux';

export default function OrderRequestOnlineCartType({ navigation, route }) {
    const { Data, Total, type } = route.params;
    const [ methodId, setMethodId]= useState(null)
    const dispatch = useDispatch();

    const { orderRequestFinishOrderLoading, finishOrderLoading  } = useSelector(state => state.cart);
        ...
        <View style={{ width: '100%', height: '100%', alignItems: 'center' }}>
            ...
            <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', margin: 8 }}>
                    <CustomButton Loading={null} onPress={() => {
                        // check for type
                        if(type=='service') dispatch(orderRequestFinishOrderAction( Total, methodId,navigation))
                         else  dispatch(finishOrderAction(methodId,navigation))
                        }} BtnTitle="سداد" borderRadius={Math.round(width / 2 + height / 2)} LinearGradientColors={['#42B5D0', '#3EB0CE', '#1579BB']}
                         Loading={type=='service'? orderRequestFinishOrderLoading: finishOrderLoading} />
                </View>
        </View>
    )
}