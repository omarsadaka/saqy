...
import style from './SanitationInfoStyle';

const { width, height } = Dimensions.get('window');


export default function SanitationInfo({ companyId, guest }) {

    const dispatch = useDispatch();

    const { companyInfoLoading, companyInfo } = useSelector(state => state.companyInfo);
        ...
        companyInfoLoading || loading? <Loading /> :
            !companyInfo ? InformationEmpty() :
                <ScrollView style={{ width: '100%', height: '100%' }} contentContainerStyle={{ alignItems: 'center', justifyContent: 'center', height: height }}>
                    <View style={{ width: '95%', alignItems: 'center', borderRadius: 5, backgroundColor: '#FFFFFF', elevation: 5, shadowColor: '#233B5D', shadowOpacity: 0.8, shadowRadius: 2, shadowOffset: { height: 1, width: 1 },marginTop:height*0.02, paddingVertical: height*0.02 }}>
                    ...
                    <View style={{ width: '95%', flex:1, borderRadius: 10, overflow: 'hidden', alignItems: 'center', marginVertical: '5%' }}>
                        <MapView
