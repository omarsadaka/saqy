...
import style from './InformationStyle';

const {width, height} = Dimensions.get('window');

export default function Information({companyId, guest}) {
  const dispatch = useDispatch();

  const {companyInfoLoading, companyInfo, companyInfoFail} = useSelector(
  ...
  return companyInfoLoading || loading ? (
    <Loading />
  ) : !companyInfo ? (
    InformationEmpty()
  ) : (
    <ScrollView
      ...
      <View
        ...
        <View style={styles.mapOvarLay} />
        <MapView
          style={{width: '100%', height: '80%', zIndex: -12}}
          scrollEnabled={false}
          onRegionChange={e => console.log('region change: ', e)}
          provider={PROVIDER_GOOGLE}
          initialRegion={{
            latitude: companyInfo.user.lat
              ? Number(companyInfo.user.lat)
              : 36.1122,
            longitude: companyInfo.user.lng
              ? Number(companyInfo.user.lng)
              : 31.1233,
          ...
          <TouchableOpacity
            onPress={() =>
              guest
                ...
                : dispatch(addItemToFavoritesAction(companyId))
            }>
