...
import { getAllCompaniesAction } from '../../../redux/TankProvidersBySubCat3/Actions';
const { width, height } = Dimensions.get('window');
import style from './MaintenanceTanksStyle';

export default function MaintenanceTanks({ navigation, fall, bottomSheetRef, subCat, title, guest }) {

    const dispatch = useDispatch();

    const { companiesTankBySubCatLoading3, companiesTankBySubCat3 } = useSelector(state => state.TankProviderBySubCat3);
    ...
    console.log("MaintenanceTanks: ", companiesTankBySubCat3?.data)
    ...
    return (
         <View style={{ width: '100%', height: '100%', alignItems: 'center' }}>
            ...
            {companiesTankBySubCatLoading3 ? <Loading /> : <View style={{ width: '90%', height: '85%' }}>
                <FlatList
                    data={companiesTankBySubCat3.data}
                    keyExtractor={item => item.id}
                    contentContainerStyle={[ { flexGrow: 1 } , companiesTankBySubCat3?.data ? null : { justifyContent: 'center'} ]}
                    ListEmptyComponent={() => (
                        <View style={{ width: '100%', height: '100%', alignItems: 'center' }}>
                    ...
                    refreshControl={
                        <RefreshControl
                          colors={['blue','gray']}
                          refreshing={companiesTankBySubCatLoading3}
